...
Fragment fragmentShaderMoon(Fragment& fragment) {
    ...
    modifiedFragment.color = Color(moonColor.x, moonColor.y, moonColor.z) * fragment.intensity;
...
Fragment fragmentShaderEarth(Fragment& fragment) {
    ...
    fragment.color = finalColor * fragment.intensity;
...
// Define la posición del centro del sol (
    ...
    float randomValue = glm::fract(glm::sin(fragment.worldPos.x * 5.0f) * rand());
